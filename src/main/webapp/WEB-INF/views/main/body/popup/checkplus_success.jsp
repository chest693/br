...
<%	//인증 후 결과값이 null로 나오는 부분은 관리담당자에게 문의 바랍니다.
    ...
    if( iReturn == 0 )
    {
        ...
        if(!sRequestNumber.equals(session_sRequestNumber))
        {
        	...
        	session.setAttribute("MBTLNUM", sMobileNo.replaceFirst("(^02|[0-9]{3})([0-9]{3,4})([0-9]{4})$", "$1-$2-$3"));
        	session.setAttribute("emplName", sName);
        }
...
$(document).ready(function() {		
	var popener = "";
	if(!window.opener){
		popener = window.open('', 'mainPopup');
	} else {
		popener = window.opener;
	}
	param = {
			"sName":"<%=sName%>"
			, "sMobileNo":"<%=sMobileNo%>"
